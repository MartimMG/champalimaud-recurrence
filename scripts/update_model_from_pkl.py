...
def write_ts(data: dict[str, Any], output_path: Path) -> None:
    ...
    lines.append("}")
    lines.append("")
    lines.append("/** Raw feature vector for SurvSHAP API (matches FEATURES order, pre-scaling 0–1 encodings). */")
    lines.append("export function encodeRawFeatureVector(input: PatientInput): number[] {")
    lines.append("  return FEATURES.map((spec) => getRawValue(input, spec));")
    lines.append("}")
    lines.append("")
    lines.append("export function buildSurvShapFeatureMeta(): { name: string; group: string }[] {")
    lines.append("  return FEATURES.map((f) => ({ name: f.name, group: f.groupName ?? f.name }));")
    lines.append("}")
    lines.append("")
